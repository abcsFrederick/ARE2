...
function parseXML($fileName)
{
  ...
  $outputFileLineROI = fopen($fileName.".lineroi", "w");
  ...
  foreach ($annotations->Annotation as $anno)
  {
    ...
    foreach ($anno->Regions->Region as $region)
    {
      ...
      if ($type == 0)
      {
        ...
        {
          $first = $roi->Vertex[0];
          $last = $roi->Vertex[count($roi->Vertex) - 1];
          #closed region
          if ( ((string)$first['X'] == (string)$last['X']) && ((string)$first['Y'] == (string)$last['Y']) )
          {
            $lineRoiArray = array();
            foreach ($roi->Vertex as $vertex)
            {
              array_push($lineRoiArray, (string)$vertex['X']);
              array_push($lineRoiArray, (string)$vertex['Y']);
            }
            array_push($lineRoiAnno, $region['DisplayId'].",".implode(",", $lineRoiArray));
          }
    ...
    foreach ($lineRoiAnno as $lineRoi)
      fwrite($outputFileLineROI, $layerNameStr.",".$lineRoi."\n");
  }

  fclose($outputFileLine);
  fclose($outputFileROI);
  fclose($outputFileLineROI);
}
  ...
  while(! feof($indexCSV))
  {
    ...
    if (file_exists($annFile))
    {
      ...
      if (filesize($annFile.".roi") != 0)
      {
        ...
        $shell_name = "aux/Aperio_Extract_Line_ROI.sh";
        ...
        exec("sbatch"." ".$shell_name." ".$annFile.".roi"." ".$roiFolder." ".$prefix." ".$postfix." ".$location." ".$reportFileName." ".$imageID." >> ".$workFolder."output.txt");
      }

      if (filesize($annFile.".lineroi") != 0)
      {
        //create output folder for current image
        $roiFolder = $outputFolder.$imageID;
        ...
        // $exec_name = "aux/Aperio_Extract_Line_ROI";
        $shell_name = "aux/Aperio_Extract_Line_ROI.sh";

        //process ROIs for current image
        fwrite($statusFile, "Extracting Line ROIs on image ".$imageID."\n");
        fflush($statusFile);
        print("sbatch"." ".$shell_name." ".$annFile.".lineroi"." ".$roiFolder." ".$prefix." ".$postfix." ".$location." ".$reportFileName." ".$imageID." >> ".$workFolder."output.txt");
        // exec($exec_name." ".$annFile.".lineroi"." ".$roiFolder." ".$prefix." ".$postfix." ".$location." ".$reportFileName." ".$imageID." >> ".$workFolder."output.txt");
        exec("sbatch"." ".$shell_name." ".$annFile.".lineroi"." ".$roiFolder." ".$prefix." ".$postfix." ".$location." ".$reportFileName." ".$imageID." >> ".$workFolder."output.txt");
      }
    }
  }
...
//  exec("chmod -R 777 ".$nfsMount.$remoteFolder);
  ...
  //20190116 fix the broken logic and only create download link when linaAnn.csv exists
  if (file_exists($workFolder."lineAnn.csv") && filesize($workFolder."lineAnn.csv") != 0)
  {
